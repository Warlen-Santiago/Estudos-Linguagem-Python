...
while True:
    ...
    if escolha == 1:
        ...
        print(f'{moeda.real(valor)} aumentado em {p}% fica : {moeda.real(moeda.aumentar(valor,p))}. ')
    elif escolha == 2:
        p = int(input(f'Em quanto por cento você quer diminuir {moeda.real(valor)}? '))
        print(f'{moeda.real(valor)} reduzido em {p}% fica: {moeda.real(moeda.diminuir(valor,p))}. ')
    elif escolha == 3:
        print(f'O dobro de {moeda.real(valor)} é {moeda.real(moeda.dobro(valor))}. ')
    elif escolha == 4:
